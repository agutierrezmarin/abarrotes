...
if not User.objects.filter(username='vendedor1').exists():
    v = User.objects.create_user('vendedor1', '', 'vendedor123', first_name='Juan', last_name='Lopez')
...
if not User.objects.filter(username='supervisor').exists():
    s = User.objects.create_user('supervisor', '', 'super123', first_name='Maria', last_name='Garcia')
...
if not User.objects.filter(username='gerente').exists():
    ge = User.objects.create_user('gerente', '', 'gerente123', first_name='Carlos', last_name='Mendoza')
...
categorias = ['Abarrotes', 'Bebidas', 'Lacteos', 'Carnes', 'Limpieza', 'Higiene Personal', 'Botanas', 'Pan y Tortillas']
for nombre in categorias:
    c, created = Categoria.objects.get_or_create(nombre=nombre)
    if created: print(f"Categoria '{nombre}' creada")
